...
def initiate_payment(params, merchant_key, salt, env):
    try:
        ...
        frappe.log_error("Error in Easebuzz initiate_payment", frappe.get_traceback())
...
def _payment(params, merchant_key, salt, env):
    ...
    if isinstance(argument_validation, dict) and argument_validation.get("status") == 0:
...
def _removeSpaceAndPreparePostArray(params):
    ...
    if split_payments := params.get("split_payments"):
        temp_dictionary["split_payments"] = json.dumps(split_payments).strip()
    if sub_merchant_id := params.get("sub_merchant_id"):
